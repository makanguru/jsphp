jQuery( document ).ready( function(){
	jQuery( '.hmr-accordion' ).accordion({
		heightStyle: "content"
	});
	
	if( jQuery( '.hmr-group2' ).length != 0 ){
		if( jQuery( '.hmr-group2' ).innerHeight() > jQuery( '.hmr-group1' ).innerHeight() )
			jQuery( '.hmr-group1' ).css({height:jQuery( '.hmr-group2' ).innerHeight() + 'px'});
		else
			jQuery( '.hmr-group2' ).css({height:jQuery( '.hmr-group1' ).innerHeight() + 'px'});
	}
	
	if( typeof( hmr_tab_index ) != 'undefined' ){
		jQuery( '.hmr-tabs' ).tabs({
			active: hmr_tab_index,
			activate: function(){
	...
	jQuery( '.hmr-group1 li.hmr-drag, .hmr-group2 li' ).draggable();
	jQuery( '.hmr-group2' ).droppable({ 
		accept: ".hmr-group1 .hmr-drag",
		hoverClass: "ui-state-highlight",
		drop: function( event, ui ) {
			ui.draggable.css({left:'auto',top:'auto'}).appendTo( this );
		}
	});
	jQuery( '.hmr-group1' ).droppable({ 
		accept: ".hmr-group2 .hmr-drag",
		hoverClass: "ui-state-highlight",
		drop: function( event, ui ) {
			if( ui.draggable.find( 'input' ).attr( 'name' ).match(/tax/i) )
				var putInto = jQuery( this ).find( 'ul.hmr-tax-ul' );
			if( ui.draggable.find( 'input' ).attr( 'name' ).match(/meta/i) )
				var putInto = jQuery( this ).find( 'ul.hmr-meta-ul' );
	...
	jQuery( document ).on( 'click', '.field.filter>div', function(){
		...
		var meta_options = jQuery( '#hmr-datasource' ).find( 'option' );
		...
		var txt = '<div id="hmr-dlg-shadow"></div><div id="hmr-dlg">';
		...
		if( attr.type == 'fulltext' ){
			...
			txt += '<label><input type="checkbox" ' + the_title_checked + ' class="hmr-array" name="contents[]" value="the_title" /> ' + objectL10n.the_title + '</label><br />';
			txt += '<label><input type="checkbox" ' + the_content_checked + ' class="hmr-array" name="contents[]" value="the_content" /> ' + objectL10n.the_content + '</label><br />';
			txt += '<label><input type="checkbox" ' + the_excerpt_checked + ' class="hmr-array" name="contents[]" value="the_excerpt" /> ' + objectL10n.the_excerpt + '</label><br />';
			jQuery( '#hmr-datasource' ).find( 'option' ).each( function(){
				if( jQuery( this ).attr( 'value' ).match( /meta\[(.*)\]/ ) ){
					...
					txt += '<label><input ' + checked + ' type="checkbox" class="hmr-array" name="contents[]" value="' + jQuery( this ).attr( 'value' ) + '" /> ' + jQuery( this ).text() + '</label><br />';
					}
			});
			jQuery( '#hmr-datasource' ).find( 'option' ).attr( 'fulltext_checked', '' );
		}
		...
		if( attr.type != 'fulltext' )
			txt += '<label>' + objectL10n.datasource + ':</label><select name="datasource">' + jQuery( '#hmr-datasource' ).html() + '</select>';
		...
		txt += '<hr />';
		txt += '<a class="button button-primary" id="hmr-save"><i class="fa fa-save"></i> ' + objectL10n.save + '</a> <a class="button" id="hmr-cancel"><i class="fa fa-times"></i> ' + objectL10n.cancel + '</a> <a class="button" id="hmr-delete"><i class="fa fa-trash-o"></i> ' + objectL10n.delete_element + '</a>';
		...
		if( options != '' ){
			var options_txt = '<input class="hmr-noTransmit" id="new_option" placeholder="' + objectL10n.enter_option_value + '" /><input class="hmr-noTransmit" id="new_option_key" placeholder="' + objectL10n.enter_option_key + '" /><a class="hmr button" id="add_option">' + objectL10n.add_option + '</a><ul id="options"></ul>';
			...
			if( typeof attr.option_key != 'undefined' ){
				for( var i = 0; i < attr.option_key.length; i++ ){
					jQuery( '<li><input class="hmr-array" type="hidden" name="option_val['+i+']" value="' + attr.option_val[i] + '" /><input class="hmr-array" type="hidden" name="option_key['+i+']" value="' + attr.option_key[i] + '" /><span>' +  attr.option_val[i] + ' (' + attr.option_key[i] + ')</span> <a class="button hmr-option-delete"><i class="fa fa-trash-o" title="' + objectL10n.delete_option + '"></i></a></li>' ).appendTo( '#options' );
				}
			}
		...
		if( attr.type == 'select' || attr.type == 'checkbox' || attr.type == 'radiobox' ){
			...
			jQuery( 'select[name="options"]' ).change( function(){
				var data = {
							action	:	'hmr-optionsearch',
							val		:	jQuery( '#hmr-dlg select[name="datasource"]' ).val()
				};
				var options_txt = '<input class="hmr-noTransmit" id="new_option" placeholder="' + objectL10n.enter_option_value + '" /><input class="hmr-noTransmit" id="new_option_key" placeholder="' + objectL10n.enter_option_key + '" /><a class="hmr button" id="add_option">' + objectL10n.add_option + '</a><ul id="options"></ul>';
							...
							function( response ){
								...
								if( response.length > 0 ){
									for( var i = 0; i < response.length; i++ )
										jQuery( '<li><input class="hmr-array" type="hidden" name="option_val['+jQuery( '#options li' ).length+']" value="' + response[i].val + '" /><input class="hmr-array" type="hidden" name="option_key['+jQuery( '#options li' ).length+']" value="' + response[i].key + '" /><span>' +  response[i].val + ' (' + response[i].key + ')</span> <a class="button hmr-option-delete"><i class="fa fa-trash-o" title="' + objectL10n.delete_option + '"></i></a></li>' ).appendTo( '#options' );
								}
								jQuery( '#options' ).sortable();
								place_dlg();
							}
		...
		if( typeof attr.datasource != 'undefined' ){
			jQuery( '#hmr-dlg select[name="datasource"] option' ).each( function(){
		...
		jQuery( '#hmr-cancel' ).click( function( event ){
			event.preventDefault();
			jQuery( '#hmr-dlg, #hmr-dlg-shadow' ).remove();
		});
		
		jQuery( '#hmr-dlg input' ).keyup( function(){
			var val = jQuery( this ).val();
			val = val.replace( /'/g, '’' );
			jQuery( this ).val( val );
		});
		
		jQuery( '#hmr-save' ).click( function( event ){
			var new_attr = {type:attr.type};
			event.preventDefault();
			element.find( 'input' ).remove();
			element.children( 'span' ).text( jQuery( '#hmr-dlg input[name="fieldname"]' ).val() );
			if( jQuery( 'select[name="options"]' ).val() == 'individual' && jQuery( '#options li' ).length == 0 ){
				alert( objectL10n.please_enter_an_option_value );
				return;
			}
			
			jQuery( '#hmr-dlg .hmr-orderby input[type="checkbox"]' ).each( function(){
				if( !jQuery( this ).prop( 'checked' ) )
					jQuery( this ).next( 'input' ).remove();				
			});
			
			jQuery( '#hmr-dlg input, #hmr-dlg select' ).each( function(){
				if( !jQuery( this ).hasClass( 'hmr-noTransmit' ) ){
					if( !jQuery( this ).hasClass( 'hmr-array' ) ){
						if( jQuery( this ).attr( 'type' ) != 'checkbox' || jQuery( this ).prop( 'checked' ) ){
							new_attr[ jQuery( this ).attr( 'name' ) ] = jQuery( this ).val();
							jQuery( '<input type="hidden" name="hmr[fields][' + fieldid + '][' + jQuery( this ).attr( 'name' ) + ']" value="' + jQuery( this ).val() + '" />' ).appendTo( element );
						}
					} else {
						...
						if( jQuery( this ).attr( 'type' ) != 'checkbox' || jQuery( this ).prop( 'checked' ) ){
							...
							jQuery( '<input type="hidden" name="hmr[fields][' + fieldid + '][' + attrkey[0] + '][]" value="' + jQuery( this ).val() + '" />' ).appendTo( element );
						}
					}
				}
			});
			element.attr( 'data-attr',JSON.stringify( new_attr ) );
			jQuery( '<input type="hidden" name="hmr[fields][' + fieldid + '][type]" value="' + attr.type + '" />' ).appendTo( element );
			jQuery( '#hmr-dlg, #hmr-dlg-shadow' ).remove();
		});
		
		jQuery( '#hmr-delete' ).click( function( event ){
			event.preventDefault();
			element.remove();
			jQuery( '#hmr-dlg, #hmr-dlg-shadow' ).remove();
		});
	...
	function place_dlg(){
		var dlg_t = Math.round( jQuery( window ).innerHeight()/2 - jQuery( '#hmr-dlg' ).outerHeight()/2 );
		var dlg_l = Math.round( jQuery( window ).innerWidth()/2 - jQuery( '#hmr-dlg' ).innerWidth()/2 );
		if( dlg_t < 25 )
			dlg_t = 25;
		if( dlg_t < 2 )
			dlg_t = 2;
		
		if( typeof jQuery( '#hmr-dlg' ).attr( 'data-place' ) != 'undefined' ){
			if( dlg_l > jQuery( '#hmr-dlg' ).offset().left )
				return;
			if( dlg_t > jQuery( '#hmr-dlg' ).offset().top )
				return;
		} else {
			jQuery( '#hmr-dlg' ).attr( 'data-place','1' );
		}
		jQuery( '#hmr-dlg' ).animate({left:dlg_l+'px',top:dlg_t+'px'});
	}
	
	jQuery( '.hmr-form-delete' ).click( function( event ){
		event.preventDefault();
	...
	jQuery( document ).on( 'click', '#options li', function( event ){
		...
		var txt = '<a href="#" title="' + objectL10n.update_option + '" class="button hmr-option-update"><i class="fa fa-save"></i></a>';
		jQuery( txt ).appendTo( this );
		jQuery( '.hmr-option-update' ).click( function( event ){	
	...
	jQuery( document ).on( 'click', '.hmr-option-delete', function(){
	...
	jQuery( document ).on( 'click','#add_option', function( event ){
						...
						var option_value = '<li><input class="hmr-array" type="hidden" name="option_val['+option_id+']" value="' + val + '" /><input class="hmr-array" type="hidden" name="option_key['+option_id+']" value="' + key + '" /><span>' +  val + ' (' + key + ')</span> <a class="button hmr-option-delete"><i class="fa fa-trash-o " title=" title="' + objectL10n.delete_option + '"></i></a></li>'
	...
	jQuery( '.hmr-colorfield' ).wpColorPicker();

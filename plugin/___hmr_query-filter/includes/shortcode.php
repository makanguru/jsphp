...
<div class="hmr-wrapper">
	...
	ul.hmr-result > li:nth-child(2n){
		margin-right: 0;
	}

	
	ul.hmr-result > li:nth-child(2n+1){
		clear: both;
	}
	...
	ul.hmr-result > li:nth-child(3n){
		margin-right: 0;
	}

	
	ul.hmr-result > li:nth-child(3n+1){
		clear: both;
	}
	...
	ul.hmr-result > li:nth-child(4n){
		margin-right: 0;
	}

	
	ul.hmr-result > li:nth-child(4n+1){
		clear: both;
	}
	<?php endif; ?>
	<?php endif; ?>
	...
	ul.hmr-nav > li > span.hmr-nav-click{
		background: <?php echo $field['background']; ?>;
	}
	<?php endif; ?>
	...
	<script>
		var hmr_columns = <?php if( isset( $field['columns'] ) ) echo $field['columns']; else echo 1;?>;
	</script>
	<div class="hmr-filter">
		<?php apply_filters( 'hmr-before-form', '' ); ?>
		...
		?>
		<fieldset data-id="<?php echo $key; ?>" <?php  echo $style_hide . 'data-condkey="' . $cond_key . '" data-condval="'  . $cond_value .  '"'; ?> class="hmr-element<?php echo $class_hide; ?> <?php echo $element['type']; ?>">
			<legend><?php echo $element['fieldname']; ?></legend>	
		<?php	
			if( $element['type'] == 'select' ):
			?>
			<select id="hmr-field-<?php echo $key; ?>" name="<?php echo $key; ?>"><option></option><?php
				...
				elseif( $data_type == 'others' ):
					if( $data_value == 'author' ):
						...
						$authors = apply_filters( 'hmr-get-authors', get_users( $args ) );
			...
			?>
				<div class="hmr-checkbox-wrapper">
				<?php 
			...
			?>
				<div class="hmr-radiobox-wrapper">
				<?php 
					if( $element['options'] == 'individual' ): ?>
						<?php foreach( $element['option_key'] as $option_key => $val ):
						?>
			...
			?>
				<div class="hmr-fulltext-wrapper">
	...
	<?php apply_filters( 'hmr-after-form', '' ); ?>
			<?php
			if( isset( $attr['showall'] ) ):
				unset( $_POST['data'] );
				$_POST['data']['search-id'] = $attr['id'];
				$results = hmr_do_search();
			endif;
		?>
		
	<div class="hmr-result-head">
		<?php
			if( isset( $results ) ):				
					echo $results['head'];
			endif;
	...
	<?php apply_filters( 'hmr-after-result-head', '' ); ?>
	...
	<?php apply_filters( 'hmr-after-results', '' ); ?>
	<ul class="hmr-nav">
		<?php
			if( isset( $results ) ):				
	...
	<?php apply_filters( 'hmr-after-navigation', '' ); ?>
</div>

		<?php
			if( isset( $results ) ):				
				?>
				<script>hmr_adjust_elements_waitimg();</script>
				<?php
			endif;
		?>

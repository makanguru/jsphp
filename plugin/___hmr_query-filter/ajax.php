<?php
	add_action('wp_ajax_hmr-deleteform', 'hmr_ajax_deleteform');
	function hmr_ajax_deleteform(){		
	...
	add_action('wp_ajax_hmr-optionsearch', 'hmr_ajax_optionsearch');
	function hmr_ajax_optionsearch(){
		$data = array();
		$i = 0;
	...
	add_action('wp_ajax_hmr-search', 'hmr_ajax_search');
	add_action('wp_ajax_nopriv_hmr-search', 'hmr_ajax_search');
	function hmr_ajax_search(){	
		error_reporting( 0 );
		echo json_encode( hmr_do_search() );
		die();	
	}
	
	function hmr_do_search( $exclude = array() ){
		global $wpdb;
			
		if( !isset( $_POST['data']['page'] ) || $_POST['data']['page'] == 1 )
			$_SESSION['hmr'] = $_POST['data'];
		...
		$template_file = HMR_DIR . 'templates/template-' . $field['name'];
		if( function_exists('is_multisite') && is_multisite() )
			$template_file = HMR_DIR . 'templates/template-' . $wpdb->blogid . '-' . $field['name'] ;
		
		
		if( !is_file( $template_file . '.php' ) )
			$template_file = HMR_DIR . 'templates/res/template-standard';
				...
				*/					
				...
				elseif( $val['type'] == 'fulltext' && !empty( $_POST['data'][ $key ]['val'] ) ):
					if( in_array( 'the_title', $val['contents'] ) )
						$args['hmr-title'] = $_POST['data'][ $key ]['val'];
					if( in_array( 'the_content', $val['contents'] ) )
						$args['hmr-content'] = $_POST['data'][ $key ]['val'];
					if( in_array( 'the_excerpt', $val['contents'] ) )
						$args['hmr-excerpt'] = $_POST['data'][ $key ]['val'];
					foreach( $val['contents'] as $v ):
						if( preg_match( '^meta\[(.*)\]^', $v ) ):
							if( !isset( $args['hmr-meta'] ) ):
								$args['hmr-meta'] = array();
							endif;
							$args['hmr-meta'][ $v ] = $_POST['data'][ $key ]['val'];
						endif;
					endforeach;
					add_filter( 'posts_where', 'hmr_content_filter', 10, 2 );
					if( isset( $args['hmr-meta'] ) )
						add_filter( 'posts_join_paged', 'hmr_content_filter_join', 10, 2 );
		...
		$data['result'] = array();
		$args = apply_filters( 'hmr-filter-args', $args );
		...
		remove_filter( 'posts_join_paged', 'hmr_content_filter_join' );
		remove_filter( 'posts_where', 'hmr_content_filter' );
		if( $query->have_posts() ):
			while( $query->have_posts() ): $query->the_post();
				...
				$data['result'][] = '<li>' . apply_filters( 'hmr-results-single-result', $template_single ) . '</li>';
			endwhile;
		endif;
		...
		if( count( $data['result'] ) == 0 ):			
			...
			ob_end_clean();
			$data['result'][] = '<li class="hmr-noresult">' . apply_filters( 'hmr-results-noresult', $template_noresult ) . '</li>';
		endif;
		...
		if( defined( 'ICL_LANGUAGE_CODE' )  ):
			global $sitepress;
			$num_of_posts = hmr_count_posts( $sitepress->get_current_language(), $field['posttype'] );
		else:
			$num_of_posts = 0;
		...
		$data['head'] = sprintf( __( '<span class="hmr-foundcount">%d results</span> out of <span class="hmr-totalcount">%d posts</span>', 'hmr' ), $query->found_posts, $num_of_posts );
		...
		if( $query->max_num_pages > 1 ):
			...
			if( $paged > 1 )
				$data['nav'][]='<li><span class="hmr-nav-click hmr-nav-left-arrow" data-href="' . ( $paged - 1 ) . '">&laquo;</span></li>';
			while( $i < $query->max_num_pages ){
				$i++;
				if( $i == 1 || ( $i > $paged - $pages_around_result && $i < $paged + $pages_around_result ) || $i == $query->max_num_pages ){
					if( $i != $paged )
						$data['nav'][]='<li><span class="hmr-nav-click" data-href="' . ( $i ) . '">' . $i . '</span></li>';
					else
						$data['nav'][]='<li><span class="hmr-nav-current">' . $i . '</span></li>';
				} elseif( ( $i == $paged - $pages_around_result || $i == $paged + $pages_around_result )  ){
						$data['nav'][]='<li><span class="hmr-nav-three-points">...</span></li>';
				}
			}
			if( $paged < $query->max_num_pages )
				$data['nav'][]='<li><span class="hmr-nav-click hmr-nav-right-arrow" data-href="' . ( $paged + 1 ) . '">&raquo;</span></li>';		
